...
const columns = ['등록', 'RMS', '작업자', '단계', '운영 반영일', '업무제목', '비고', '업무 시작 시간', '업무 종료 시간', '작업시간', '조정'];
...
function makeSnapshot(csv) {
  ...
  const tasks = rawRows.slice(1).flatMap((row, index) => {
    ...
    const values = columns.map(name => {
      ...
      const legacyIndex = name === '작업시간' ? headers.indexOf('실 작업시간') : name === '운영 반영일' ? headers.indexOf('완료 & 반영일') : -1;
